...
export default function RoundScore({
  ...
  return (
    /* Fullscreen viewport wrapper to guarantee vertical & horizontal centering */
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-auto">
      <motion.div
        ...
        className="w-[320px] md:w-[600px] lg:w-[650px] h-[97vh] p-5 bg-game-panel text-white rounded-lg border-2 border-slate-800 text-sm md:text-base overflow-y-auto flex flex-col justify-between"
      >
        <div>
          <div className="relative flex items-center justify-center mb-3">
            <h2 className="text-lg md:text-2xl text-white text-center">Round Summary</h2>
            {!showFinal && (
              <button
            ...
            {raceStarted && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-3">
                <div className="flex justify-around items-center">
                  <motion.p className="text-xl md:text-3xl font-bold text-cyan-300 tabular-nums">
                    {Math.round(raceRow)}
                  </motion.p>
              ...
              <div className="bg-black/25 rounded-lg p-3 border border-white/10">
                <div className="flex justify-around">
                  <div className="text-center">
                    ...
                    <p className="text-xl md:text-2xl font-bold text-white">{totalScores[0]}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-fuchsia-400 font-bold">Column</p>
                    <p className="text-xl md:text-2xl font-bold text-white">{totalScores[1]}</p>
                  </div>
                </div>
              </div>

              <button
                className="w-full bg-blue-500 text-white font-bold rounded-xl md:text-lg border-white border-2 hover:bg-blue-600 py-2 mt-3 transition-colors duration-300 cursor-pointer"
                onClick={() => {
                  playDiscardSound();
                  nextRound();
                }}
