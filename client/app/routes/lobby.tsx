  ...
  <ul className="list-disc list-inside text-white mb-6">
          {lobby.players.map((player: any) => (
            <li key={player.id}>
              ...
              {player.name}
              {player.id === lobby.host && <span className="badge-host px-2 rounded-full text-xs ml-2">Host</span>}
              {player.id === myId && <span className="badge-you px-2 rounded-full text-xs ml-2">You</span>}
            </li>
          ))}
        </ul>
